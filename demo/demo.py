...
def get_parser():
    ...
    parser.add_argument(
        "--optimize", action="store_true", help="whether optimize for the camera param."
    )
    parser.add_argument(
        "--net-init",
        action="store_true",
        help="whether use network init during optimization.",
    )
    parser.add_argument(
        "--noncenter",
        action="store_true",
        help="Whether assume centered principal point during optimization.",
    )
    parser.add_argument(
        "--input",
        nargs="+",
...
def resize_img(img, min_size, max_size):
    ...
    Args:
        ...
        min_size (int, int): min size to resize to
        max_size (int, int): max size to resize to
...
def resize_fix_aspect_ratio(img, field, target_width=None, target_height=None):
    ...
    Args:
        ...
        target_width (_type_, optional): _description_. Defaults to None.
        target_height (_type_, optional): _description_. Defaults to None.
...
def save_gif(demo, img, pred, output_folder):
    """save gif of perspective field predictions

    Args:
        demo (VisualizationDemo):
        img (np.ndarray): input image
        pred (dict): model predictions
        output_folder (str): path to save gif to
    """
    field = {
    ...
    for frame_id in range(num_frames):
        ...
        rendered[int(i * bottom.shape[0]) :, :, :] = (1 - alpha) * bottom[
            int(i * bottom.shape[0]) :, :, :
        ] + alpha * up[int(i * bottom.shape[0]) :, :, :]
...
def save_vis(demo, img, pred, output_folder):
    ...
    pred_vis: Perspective field prediction from PerspectiveNet
    param_vis: Perspective field prediction derived from camera params predicted by ParamNet
    optimized_vis: Perspective field prediction derived from optimized camera params (if args.optimize == True)

    Args:
        demo (VisualizationDemo):
        img (np.ndarray): input image
        pred (dict): model predictions
        output_folder (str): path to save visualizations to
    """
    field = {
        "up": pred["pred_gravity_original"].cpu().detach(),
        "lati": pred["pred_latitude_original"].cpu().detach(),
    ...
    if "opt_param" in pred.keys():
        ...
        cv2.imwrite(
            os.path.join(output_folder, "opt_param.jpg"), optimized_vis[:, :, ::-1]
        )
        # write as json file
...
if __name__ == "__main__":
    ...
    if args.input:
        ...
        for path in tqdm.tqdm(args.input, disable=not args.output):
            ...
            if args.optimize:
                predictions["opt_param"] = demo.opt_rpfpp(
                    predictions, "cpu", net_init=args.net_init, pp_on=args.noncenter
                )
