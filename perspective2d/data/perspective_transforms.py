...
class PerspectiveMapper:
    ...
    def __init__(self, cfg, is_train=True, dataset_names=None):
        ...
        Args:
            ...
            dataset_names (str, optional): Defaults to None.
        """
    ...
    def init_pseudo_uniform_counter(self):
        """
        Initialize parameter ranges and parameter sampling buckets
        """
    ...
    def sample_general_vfov(self, max_vfov):
        """Used to uniformly sample vertical field of views

        Args:
            max_vfov (float): maximum vertical field of view that can be sampled

        Returns:
            float: sampled vertical field of view
        """
        ...
        selected_bin = np.random.choice(
            np.where(
                np.min(self.sampled_general_vfov_count[:max_bin_id])
                == self.sampled_general_vfov_count[:max_bin_id]
            )[0],
            1,
        )[0]

        bin_size = (
            self.general_vfov_range[1] - self.general_vfov_range[0]
        ) / self.general_vfov_range[2]
        self.sampled_general_vfov_count[selected_bin] += 1
        vfov = min(
            max_vfov,
            self.decode_param_bin(selected_bin, self.general_vfov_range)
            + np.random.uniform(-bin_size / 2, bin_size / 2),
        )
        return vfov.item()

    def sample_rel_f(self, min_rel_f):
        """Used to uniformly sample relative focal lengths

        Args:
            min_rel_f (float): minimum relative focal length that can be sampled

        Returns:
            float: sampled relative focal length
        """
        min_bin_id = self.encode_param_bin(min_rel_f, self.rel_focal_range)
        selected_bin = np.random.choice(
            np.where(
                np.min(self.sampled_rel_f_count[min_bin_id:])
                == self.sampled_rel_f_count
            )[0],
            1,
        )[0]

        bin_size = (
            self.rel_focal_range[1] - self.rel_focal_range[0]
        ) / self.rel_focal_range[2]
        self.sampled_rel_f_count[selected_bin] += 1
        rel_f = max(
            min_rel_f,
            self.decode_param_bin(selected_bin, self.rel_focal_range)
            + np.random.uniform(-bin_size / 2, bin_size / 2),
        )
        return rel_f
    ...
    def encode_param_bin(self, value, param_range):
        ...
        Args:
            value (float): value to be encoded
            param_range (int, int, int): (min value, max value, num bins)
    ...
    def decode_param_bin(self, bin_id, param_range):
        ...
        bin_size = (param_range[1] - param_range[0]) / param_range[2]
        bin_centers = (
            torch.arange(param_range[0], param_range[1], bin_size) + bin_size / 2
        )
    ...
    def __call__(self, dataset_dict):
        ...
        if self.focal_on:
            if self.is_train and not self.overfit_on:
                ...
                focal_length = (
                    img_height / 2 / np.tan(np.radians(dataset_dict["vfov"] / 2))
                )
            else:
                assert (
                    "vfov" not in dataset_dict.keys()
                )  # should test on cropped images?
        ...
        if self.latitude_on:
            (
                gt_latitude_original,
                gt_latitude_original_mode,
            ) = self.latitude_transform.get_input_label(dataset_dict)
        else:
            gt_latitude_original, gt_latitude_original_mode = None, None

        if self.gravity_on:
            gt_absvvp, gt_gfield_original = self.gravity_transform.get_input_label(
                dataset_dict
            )
        else:
        ...
        if self.debug_on:
            if not self.overfit_on:
                dataset_dict["img_center_original"] = cv2.circle(
                    img.copy(),
                    pp.astype(int),
                    radius=10,
                    color=(0, 0, 255),
                ...
                original_path = os.path.join(
                    "/home/jinlinyi/tmp/google_street_view_191210/manhattan",
                    *(dataset_dict["file_name"].split("/")[-1].split("-", 1))
                )
                ...
                dataset_dict["img_center_original"] = cv2.circle(
                    original_rgb.copy(),
                    original_rgb_center.astype(int),
                    radius=10,
                    color=(0, 0, 255),
                    thickness=-1,
                )
        ...
        if self.cfg.DATALOADER.AUGMENTATION_FUN == "with_up_field":
            img, gt_latitude_aug, gt_gfield_aug = self.augmentation_with_up_field(
                img, gt_latitude_original, gt_gfield_original
            )
            pp_aug, focal_length_aug = (0, 0), 0
        elif (
            self.cfg.DATALOADER.AUGMENTATION_FUN == "default"
            ...
            (
                ...
                img, gt_latitude_original, gt_absvvp, pp, focal_length
            )
            ...
            if self.pp_on:
                ...
                focal_length = (
                    dataset_dict["height"]
                    / 2
                    / np.tan(np.radians(dataset_dict["vfov"] / 2))
                )
                (
                    img,
                    gt_latitude_aug,
            ...
            (
                ...
                pp_aug,
                focal_length_aug,
            ) = self.uniform_rel_f_crop_resize(
                img, gt_latitude_original, gt_absvvp, pp, focal_length
            )
        else:
        ...
        if self.gravity_on:
            if self.cfg.DATALOADER.AUGMENTATION_FUN == "with_up_field":
                dataset_dict[
                    "gt_gravity"
                ] = self.gravity_transform.to_tensor_from_field(
                    im_h=img.shape[0], im_w=img.shape[1], gfield=gt_gfield_aug
                )
            else:
                dataset_dict["gt_gravity"] = self.gravity_transform.to_tensor(
                    im_h=img.shape[0], im_w=img.shape[1], absvvp=gt_absvvp_aug
                )

        if self.latitude_on:
            dataset_dict["gt_latitude"] = self.latitude_transform.to_tensor(
                gt_latitude_aug
            )
        ...
        if rel_focal is not None:
            dataset_dict["general_vfov"] = general_vfov(
                rel_pp[0], rel_pp[1], 1, rel_focal, degree=True
            )

        if not self.is_train:
            if self.gravity_on:
                # Get gfield for original image
                gt_gfield_original = torch.as_tensor(
                    gt_gfield_original.transpose(2, 0, 1).astype("float32")
                )
                dataset_dict["gt_gravity_original"] = gt_gfield_original
            if self.latitude_on:
                gt_latitude_original = torch.as_tensor(
                    gt_latitude_original.astype("float32")
                )
    ...
    def augmentation_with_up_field(self, img, latimap, gfield):
        ...
        if self.is_train:
            ...
            aug_list.append(A.Resize(self.resize[0], self.resize[1]))
        ...
        gfield_aug = normalize(gfield_aug.reshape(-1, 2)).reshape(
            transformed["upmap"].shape
        )
    ...
    def augmentation(self, img, latimap, absvvp, pp, focal_length):
        """Default augmentation function, calls self.aug to perform augmentations

        Args:
            image (np.ndarray): input image
            latimap (np.ndarray): ground truth latitude map
            absvvp ([float, float, float]): ground truth absolute vertical vanishing point
            pp ([int, int]): ground truth principle point location
            focal_length (float): ground truth focal length

        Returns:
            img_aug (np.ndarray): augmented image
            latimap_aug (np.ndarray): augmented latitude map
            absvvp_aug ([float, float, float]): absolute vertical vanishing point of augmented image
            pp_aug ([float, float]): principle point location of augmented image
            focal_length_aug (float): focal length of augmented image
        """
        h, w, _ = img.shape
        ...
        absvvp_aug = (
            np.array(
                [
                    transformed["keypoints"][0][0],
                    transformed["keypoints"][0][1],
                    absvvp[2],
                ]
            )
            if absvvp is not None
            else None
        )
        ...
        if focal_length is not None:
            ...
            focal_length_aug = (
                focal_length
                * (transformed["corner"][2] - transformed["corner"][0])[1]
                / h
            )
        else:
    ...
    def uniform_vfov_crop_resize(self, image, latimap, absvvp, pp, focal_length):
        """Augmentation function
           If self.pp_on, crop images with random relative principal points and uniformly sampled vertical field of views.
           If not self.pp_on, crop images with centered principal points and uniformly sampled vertical field of views.
        ...
        Returns:
            img_aug (np.ndarray): cropped image
            latimap_aug (np.ndarray): cropped latitude map
            absvvp_aug ([float, float, float]): absolute vertical vanishing point of cropped image
            pp_aug ([float, float]): principle point location of cropped image
            focal_length_aug (float): focal length of cropped image (None if not self.pp_on)
            vfov (float): vertical field of view of cropped image (only if not self.pp_on)
        """
        ...
        max_vfov = general_vfov(
            rel_cx, rel_cy, 1, focal_length / crop_size_max, degree=True
        )
        vfov = self.sample_general_vfov(max_vfov)
        h = scipy.optimize.fsolve(
            fun,
            crop_size_max,
            args=(focal_length, rel_cx, rel_cy, np.cos(np.radians(vfov))),
        )[0]
        h = int(np.abs(h))
        w = h
        cx = rel_cx * w + 0.5 * W
        ...
        latimap_aug = (
            cv2.resize(transformed["latimap"], self.resize)
            if latimap is not None
            else None
        )
    ...
    def uniform_rel_f_crop_resize(self, image, latimap, absvvp, pp, focal_length):
        """Augmentation function
           Crop images to produce training examples with off-centered principle points.
           Randomly select relative principal point location and uniformly sample relative
           focal length.
        ...
        Returns:
            ...
            absvvp_aug ([float, float, float]): absolute vertical vanishing point of cropped image
            pp_aug ([float, float]): principle point location of cropped image
            focal_length_aug (float): focal length of cropped image
        """
        image = self.color_aug(image=image)["image"]
        H, W, _ = image.shape
        assert H == W
            ...
            if self.cfg.MODEL.RECOVER_RPF:
                ...
                aug_list.extend(
                    [
                        ...
                        A.RandomResizedCrop(
                            height=self.resize[0], width=self.resize[1]
                        ),
                    ]
                )
