...
class ParamNet(nn.Module):
    ...
    def forward(self, predictions, batched_inputs=None):
        ...
        if self.cfg.MODEL.RECOVER_RPF:
            if self.cfg.MODEL.RECOVER_PP:
                ...
                targets = torch.FloatTensor([np.array([x["roll"] / 90.0, x["pitch"] / 90.0, x["rel_focal"]]) for x in batched_inputs])
            else:
                # roll [-90, 90]->(-1,1), pitch [-90, 90]->(-1,1), vfov -> vfov / 90
                targets = torch.FloatTensor([np.array([x["roll"] / 90.0, x["pitch"] / 90.0, x["vfov"] / 90.0]) for x in batched_inputs])
    ...
    def losses(self, pred, gt):
        ...
        if self.cfg.MODEL.PARAM_DECODER.LOSS_TYPE == 'regression':
            ...
            if self.cfg.MODEL.RECOVER_PP:
                loss_itemized = F.mse_loss(pred, gt, reduction='none') * mask * self.loss_weight
    ...
    def visualize(self, predictions, batched_inputs):
        ...
        vis_img = cv2.circle(batched_inputs[0]['image'].cpu().numpy().transpose(1,2,0).copy(), pp_gt.astype(int), radius=10, color=(0, 0, 255), thickness=-1)
        pp_pred = param['pred_rel_pp'].cpu().numpy() * h + np.array([w, h]) / 2
        vis_img = cv2.circle(vis_img, pp_pred[0].astype(int), radius=8, color=(0, 255, 0), thickness=-1)
        
        original = cv2.resize(batched_inputs[0]['img_center_original'], (vis_img.shape[:2]))
...
class ParamNetConvNextRegress(nn.Module):
    ...
    def visualize(self, predictions, batched_inputs):
        ...
        param['pred_rel_pp'] = torch.cat([param['pred_rel_cx'].view(-1, 1), param['pred_rel_cy'].view(-1, 1)], dim=-1) 
        vis_dict = {}
        _, h, w = batched_inputs[0]['image'].shape
        pp_gt = batched_inputs[0]['rel_pp'] * h  + np.array([w, h]) / 2
        vis_img = cv2.circle(batched_inputs[0]['image'].cpu().numpy().transpose(1,2,0).copy(), pp_gt.astype(int), radius=10, color=(0, 0, 255), thickness=-1)
