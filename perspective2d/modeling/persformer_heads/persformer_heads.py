...
class StandardPersformerHeads(torch.nn.Module):
    ...
    def forward(
        ...
        prediction = {}
        if self.gravity_on:
            prediction['pred_gravity'], loss_gravity = self.gravity_head(features, targets['gt_gravity'])
            losses.update(loss_gravity)
        if self.latitude_on:
            prediction['pred_latitude'], loss_latitude = self.latitude_head(features, targets['gt_latitude'])
    ...
    def postprocess(self, results, batched_inputs, images):
        processed_results = []
        if self.gravity_on:
            processed_gravity = self.gravity_head.postprocess(results['pred_gravity'], batched_inputs, images)
        else:
            processed_gravity = [{} for _ in batched_inputs]
        
        if self.latitude_on:
            processed_latitude = self.latitude_head.postprocess(results['pred_latitude'], batched_inputs, images)
        else:
    ...
    def visualize(self, img, feature, target):
        ...
        if self.gravity_on:
            # Score maps
            vis_dict[f"gravity-score-map"] = self.visualize_scoremap(results['pred_gravity'])
    ...
    def visualize_scoremap(pred):
        ...
        score_maps = F.interpolate(score_maps.unsqueeze(0), size=(score_maps.size(1)//4, score_maps.size(2)//4), mode='bilinear', align_corners=False)[0]
        return score_maps