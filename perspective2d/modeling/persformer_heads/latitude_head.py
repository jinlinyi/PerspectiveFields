...
class LatitudeDecoder(BaseDecodeHead):
    ...
    def __init__(self, feature_strides, loss_weight, **kwargs):
        super(LatitudeDecoder, self).__init__(input_transform='multiple_select', **kwargs)
    ...
    def postprocess(self, results, batched_inputs, images):
        ...
        for result, input_per_image, image_size in zip(results, batched_inputs, images.image_sizes):
            ...
            if self.loss_type == 'regression':
                ...
                latimap = decode_bin_latitude(latimap_bin, self.num_classes).unsqueeze(0)
                latimap = sem_seg_postprocess(latimap, image_size, height, width)[0]
            else:
                raise NotImplementedError
            processed_results.append({"pred_latitude": result, "pred_latitude_original": latimap, "pred_latitude_original_mode": "deg"})
    ...
    def losses(self, predictions, targets):
        ...
        if self.loss_type == 'regression':
            ...
            losses['latitude-msg-normal-loss'] = 0.1 * msgil_norm_loss(predictions, targets, mask) * self.loss_weight
            losses['latitude-l2-loss'] = F.mse_loss(predictions, targets, reduction='none')[mask].mean() * self.loss_weight
    ...
    def visualize(self, img, pred, gt):
        if self.loss_type == 'regression':
            ...
            pred = draw_latitude_field(
                img_rgb=img.numpy().transpose(1,2,0).astype(np.uint8), 
                # binmap=pred.cpu()
                latimap=np.radians(decode_bin_latitude(pred.cpu(), self.num_classes)),
                ) / 255.
            gt = draw_latitude_field(
                img_rgb=img.numpy().transpose(1,2,0).astype(np.uint8), 
                # binmap=gt.cpu()
