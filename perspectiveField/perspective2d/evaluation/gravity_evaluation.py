...
class GravityEvaluator:
    ...
    def process(self, input, output):
        ...
        if "pred_gravity_original" in output.keys():
            ...
            gt = input['gt_gravity_original'].flatten(start_dim=1).unsqueeze(0).to(pred.device)
            ...
            ret['gravity_angle_err_mean'] = torch.mean(angle_err.to(self._cpu_device)[mask]).numpy()
            ret['gravity_angle_err_med'] = torch.median(angle_err.to(self._cpu_device)[mask]).numpy()
        if "pred_gravity" in output.keys():
            if self.loss_type == 'regression':
                pred = output['pred_gravity']
                gt = input['gt_gravity'].to(pred.device)
                loss = F.l1_loss(
                    pred.unsqueeze(0), gt.unsqueeze(0), reduction="mean",
                )
                ...
                loss = F.cross_entropy(
                    pred.unsqueeze(0), gt.unsqueeze(0), reduction="mean", ignore_index=self._ignore_label
                )
            else:
                raise NotImplementedError
    ...
    def evaluate(self, predictions):
        ...
        * Pixel Accuracy (pACC)
        """
        res = {}
        res["meanAngle"] = np.average([e['gravity_angle_err_mean'] for e in predictions])
        # res["medianAngle"] = np.average([e['gravity_angle_err_med'] for e in predictions])
        res["medianAngle"] = np.median([e['gravity_angle_err_mean'] for e in predictions])
